...
public class GloModem {
    ...
    private  void initialize() throws Exception {
        ...
        gateway = new SerialModemGateway("GLO", comPort, baudRate, strmanufacturer, model);                
        gateway.getATHandler().setStorageLocations("SMME");                	                
        gateway.setProtocol(AGateway.Protocols.PDU);                
        gateway.setInbound(true);
        gateway.setOutbound(true);
        gateway.setSimPin("0000");
        gateway.setSmscNumber("+2348050001501");
        ...
        USSDRequest request = new USSDRequest(USSDResultPresentation.PRESENTATION_ENABLED, "#124#", USSDDcs.UNSPECIFIED_7BIT, gateway.getGatewayId());
    ...
    public GloModem(int baudRate, String model, String strmanufacturer, String comPort) {
    ...
    public void doIt() throws Exception {        
        //TRANSFER ---  *131*Glo Number*Airtime Amount*Password#
        //BALANCE  ---  #124*1#   
        ...
        if(tranx!=null){
        String ussdString ="*131*"+tranx.getDestinationNumber()+"*"+tranx.getAmount()+"*00000#";            
    ...
    public class USSDNotification implements IUSSDNotification {
        ...
        @Override
        public void process(AGateway lgateway, USSDResponse response) {
            System.out.println("USSD handler called from Gateway: " + lgateway.getGatewayId());
...
//                    System.out.println("BALANCE:"+lgateway.queryBalance());
//                    System.out.println("SELF:"+lgateway.getMyself());                    
                    lgateway.setInbound(true);
                    lgateway.setOutbound(true);
                    lgateway.setProtocol(AGateway.Protocols.PDU); 
                    
                    //gateway.sendCustomATCommand("AT+CUSD=1,\"1\",15\r"); 
                    //gateway.sendUSSDCommand("1",true); 
                   // gateway.sendUSSDCommand("1"); 
                    lgateway.sendUSSDCommand("1");
                    
                    
                }
                //SECOND CHANCE ENDS HERE                              
            }  catch (Exception ex) {
                Logger.getLogger(GloModem.class.getName()).log(Level.SEVERE, null, ex);
            }
            DONE=true;
        }
    }
    ...
    {       
        GloModem app = new GloModem(11200, "HSDPA Modem", "WCDMA", "COM24");
        try {
            app.initialize();
            while(true){
            app.doIt();
